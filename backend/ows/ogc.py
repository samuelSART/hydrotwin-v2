...
def ogc_update_impl():
    import os
    # result = os.system('datacube-ows-update --schema --role agdc_admin && datacube-ows-update --views && datacube-ows-update')
    # result = os.system('datacube-ows-update --views && datacube-ows-update && || true /bin/kill -s SIGTERM 1') # For Flask, it will force a container restart
    result = os.system('datacube-ows-update --views && datacube-ows-update || true && /bin/kill -s SIGHUP 1') # For Gunicorn, it will restart server process
    return "Finished" if result else "Update did not finish correctly. Please check logs."
